...
export function Matrix({
  ...
  return (
    <div>
      <div
        className="relative w-full rounded-[--radius-kaart] border border-[--color-rand] bg-[--color-vlak]"
        style={{ height: hoogte }}
      >
        {/* Kwadrantscheiding op het midden van de schaal. */}
        <div className="absolute inset-x-0 top-1/2 h-px bg-[--color-rand]" />
        <div className="absolute inset-y-0 left-1/2 w-px bg-[--color-rand]" />

        <span className="pointer-events-none absolute left-2 top-2 text-[10px] font-medium uppercase tracking-wide text-[--color-inkt-licht]">
          Strategisch investeren
        </span>
        <span className="pointer-events-none absolute right-2 top-2 text-[10px] font-medium uppercase tracking-wide text-[--color-inkt-licht]">
          Snel doen
        </span>
        <span className="pointer-events-none absolute bottom-2 left-2 text-[10px] font-medium uppercase tracking-wide text-[--color-inkt-licht]">
          Niet nu
        </span>
        <span className="pointer-events-none absolute bottom-2 right-2 text-[10px] font-medium uppercase tracking-wide text-[--color-inkt-licht]">
          Meenemen
        </span>
          ...
          return (
            <button
              ...
              className={`absolute -translate-x-1/2 translate-y-1/2 rounded-full border-2 transition-all ${
                actief
                  ? "z-10 h-5 w-5 border-[--color-accent-diep] bg-[--color-accent]"
                  : "h-3.5 w-3.5 border-[--color-vlak] bg-[--color-accent] hover:h-5 hover:w-5"
              }`}
      ...
      <div className="mt-1.5 flex justify-between text-[11px] text-[--color-inkt-licht]">
        <span>← lastiger haalbaar</span>
        <span>makkelijker haalbaar →</span>
      </div>
      <p className="mt-0.5 text-[11px] text-[--color-inkt-licht]">
        Verticaal: meer waarde naar boven. {beelden.length - geplaatst.length > 0
          ? `${beelden.length - geplaatst.length} use cases staan er nog niet op omdat ze niet gewaardeerd zijn.`
          : ""}
      </p>
    </div>
  );
...
export function KwadrantAdvies({ kwadrant }: { kwadrant: KwadrantId }) {
  ...
  return (
    <span className="text-[11px] leading-snug text-[--color-inkt-licht]">
